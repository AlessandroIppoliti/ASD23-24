...
public class EquazioneSecondoGradoModificabileConRisolutore {
    ...
    public void setA(double a) {
        ...
        if (Math.abs(a) < EPSILON) {
            throw new IllegalArgumentException("Il coefficiente 'a' non può essere zero");
        }
        this.a = a;
        this.solved = false;
    }
    ...
    public void solve() {
        ...
        if (!this.isSolved()) {
            ...
            if (delta < 0) {
                ...
                // Una soluzione
                this.lastSolution = new SoluzioneEquazioneSecondoGrado(new EquazioneSecondoGrado(this.a, this.b, this.c), (this.b) / (2 * this.a));
            } else {
                double tmp = Math.sqrt(delta);
                // Due soluzioni
                this.lastSolution = new SoluzioneEquazioneSecondoGrado(new EquazioneSecondoGrado(this.a, this.b, this.c), (-this.b + tmp) / (2 * this.a),
                        (-this.b - tmp) / (2 * this.a));
    ...
    public SoluzioneEquazioneSecondoGrado getSolution() {
        // TODO implementare
        if (!isSolved()) throw new IllegalStateException("L'equazione non è stata risolta");
        return lastSolution;
    }
