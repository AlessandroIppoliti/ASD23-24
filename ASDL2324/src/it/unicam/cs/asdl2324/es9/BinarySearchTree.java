...
public class BinarySearchTree<E extends Comparable<E>> {
    ...
    protected class RecBST {
        ...
        protected int computeHeight() {
            ...
            if (this == null) {
                return -1; // Un nodo null non ha altezza (-1)
            }

            int leftHeight = (left != null) ? left.computeHeight() : -1;
            int rightHeight = (right != null) ? right.computeHeight() : -1;
        ...
        protected boolean insert(E label) {
            ...
            if (label.compareTo(this.label) < 0) {
                if (left == null) {
                    left = new RecBST(label);
                    return true; // Inserimento avvenuto con successo
                } else {
                    return left.insert(label); // Prosegui la ricerca nel sottoalbero sinistro
                }
            } else if (label.compareTo(this.label) > 0) {
                if (right == null) {
                    right = new RecBST(label);
                    return true; // Inserimento avvenuto con successo
                } else {
                    return right.insert(label); // Prosegui la ricerca nel sottoalbero destro
                }
            } else {
                return false; // Etichetta già presente, non fare nulla
            }
        }
        ...
        protected RecBST search(E label) {
            ...
            if (label.equals(this.label)) {
                return this; // Restituisce il nodo corrente se l'etichetta è quella cercata
            } else if (label.compareTo(this.label) < 0 && left != null) {
                return left.search(label); // Prosegue la ricerca nel sottoalbero sinistro
            } else if (label.compareTo(this.label) > 0 && right != null) {
                return right.search(label); // Prosegue la ricerca nel sottoalbero destro
            } else {
                return null; // Etichetta non trovata
            }
        }
        ...
        protected void addLabelsInOrder(List<E> l) {
            // TODO implementare ricorsivamente
            if (left != null) {
                left.addLabelsInOrder(l); // Aggiungi le etichette del sottoalbero sinistro
            }

            l.add(this.label); // Aggiungi l'etichetta del nodo corrente

            if (right != null) {
                right.addLabelsInOrder(l); // Aggiungi le etichette del sottoalbero destro
            }
        }
        ...
        protected List<E> inOrderVisit() {
            ...
            if (left != null) {
                result.addAll(left.inOrderVisit()); // Visita il sottoalbero sinistro
            }
            result.add(label); // Aggiunge l'etichetta del nodo corrente
            if (right != null) {
                result.addAll(right.inOrderVisit()); // Visita il sottoalbero destro
            }
            return result;
        }
        ...
        protected RecBST getMinNode() {
            // TODO implementare ricorsivamente
            if (left != null) {
                return left.getMinNode(); // Continua a cercare nel sottoalbero sinistro
            }
            return this;
        }
        ...
        protected RecBST getMaxNode() {
            // TODO implementare ricorsivamente
            if (right != null) {
                return right.getMaxNode(); // Continua a cercare nel sottoalbero destro
            }
            return this;
        }
        ...
        protected RecBST getSuccessorNode() {
            // TODO implementare
            if (right != null) {
                // Se esiste un sottoalbero destro, il successore sarà il nodo più a sinistra in quel sottoalbero
                return right.getMinNode();
            } else {
                // Se non c'è un sottoalbero destro, risaliamo l'albero per trovare il primo antenato a sinistra
                RecBST current = this;
                RecBST parent = getParent();
        ...
        protected RecBST getPredecessorNode() {
            // TODO implementare
            if (left != null) {
                // Se esiste un sottoalbero sinistro, il predecessore sarà il nodo più a destra in quel sottoalbero
                return left.getMaxNode();
            } else {
                // Se non c'è un sottoalbero sinistro, risaliamo l'albero per trovare il primo antenato a destra
                RecBST current = this;
                RecBST parent = getParent();
        ...
        protected void deleteSelfLabel() {
            ...
            if (left == null && right == null) {
                // Caso 1: Il nodo corrente non ha figli
                if (parent != null) {
                    if (this == parent.left) {
                        parent.left = null;
                ...
                // Caso 2: Il nodo corrente ha entrambi i figli
                RecBST successor = getSuccessorNode();
                label = successor.label;
                successor.deleteSelfLabel();
            } else {
                // Caso 3: Il nodo corrente ha solo un figlio
                RecBST child = (left != null) ? left : right;
                if (parent != null) {
                    if (this == parent.left) {
                        parent.left = child;
                    } else if (this == parent.right) {
