...
public interface ADTConsList<E> {
    ...
    default ADTConsList<E> removeAll(E element) {
        ...
        if (this.isEmpty()) {
            ...
            // caso ricorsivo
            if (this.first().equals(element)) {
                // Se l'elemento corrente è uguale all'elemento da rimuovere,
                // passa alla coda per rimuovere tutte le occorrenze di element
                return this.rest().removeAll(element);
            } else {
                // Se l'elemento corrente non deve essere rimosso,
                // mantienilo e continua a cercare nelle restanti parti della lista
    ...
    default ADTConsList<E> updateFirst(E element, E newElement) {
        // TODO implementare ricorsivamente
        return updateFirstHelper(element, newElement, false);
    }

    default ADTConsList<E> updateFirstHelper(E element, E newElement, boolean found) {
        // caso base
        if (this.isEmpty()) {
            return this;
        } else {
            // caso ricorsivo
            if (this.first().equals(element) && !found) {
                // Se l'elemento corrente è uguale all'elemento da aggiornare e non è già stata trovata occorrenza,
                // sostituiscilo con il nuovo elemento e imposta il flag "trovato" a true
                return this.rest().updateFirstHelper(element, newElement, true).cons(newElement);
            } else {
                // Se l'elemento corrente non deve essere aggiornato o è già stata trovata occorrenza,
                // mantienilo e continua a cercare nelle restanti parti della lista
                return this.rest().updateFirstHelper(element, newElement, found).cons(this.first());
            }
        }
    }
    ...
    default ADTConsList<E> updateAll(E element, E newElement) {
        ...
        if (this.isEmpty()) {
            ...
            // Caso ricorsivo
            if (this.first().equals(element)) {
                // Se l'elemento corrente è uguale a quello da aggiornare,
                // sostituiscilo con il nuovo elemento e passa alla coda
                return this.rest().updateAll(element, newElement).cons(newElement);
            } else {
                // Se l'elemento corrente non deve essere aggiornato,
                // mantienilo e continua a cercare nelle restanti parti della lista
    ...
    default ADTConsList<E> append(ADTConsList<E> list) {
        // TODO implementare ricorsivamente
        // Caso base: se la lista corrente è vuota, restituisci la lista data
        if (this.isEmpty()) {
            return list;
        } else {
            // Combina il primo elemento della lista corrente
            // con il risultato dell'append sulla coda della lista corrente
    ...
    default ADTConsList<E> reverse() {
        // TODO implementare ricorsivamente
        return reverseHelper(this, EMPTY_LIST);
    }

    default ADTConsList<E> reverseHelper(ADTConsList<E> original, ADTConsList<E> reversed) {
        if (original.isEmpty()) {
            return reversed;
        } else {
            return reverseHelper(original.rest(), reversed.cons(original.first()));
        }
    }
