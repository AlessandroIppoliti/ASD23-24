...
public class BinarySearchTree<E extends Comparable<E>> {
    ...
    protected class RecBST {
        ...
        protected int computeHeight() {
            ...
            // Se il nodo è una foglia, l'altezza è 0
            if (this.left == null && this.right == null) {
                return 0;
            }

            // Calcola l'altezza del sottoalbero sinistro e destro
            int leftHeight = (this.left != null) ? this.left.computeHeight() : -1;
            int rightHeight = (this.right != null) ? this.right.computeHeight() : -1;

            // Restituisce l'altezza massima tra i sottoalberi sinistro e destro, incrementata di 1
        ...
        protected boolean insert(E label) {
            ...
            // Inserisce nell'albero ordinato
            if (label.compareTo(this.label) < 0) {
                if (this.left == null) {
                    // Se non esiste un sottoalbero sinistro, crea un nuovo nodo
                    this.left = new RecBST(label, null, null, this);
                    return true;
                } else {
                    // Continua la ricerca ricorsiva nel sottoalbero sinistro
                    return this.left.insert(label);
                }
            } else {
                if (this.right == null) {
                    // Se non esiste un sottoalbero destro, crea un nuovo nodo
                    this.right = new RecBST(label, null, null, this);
                    return true;
                } else {
                    // Continua la ricerca ricorsiva nel sottoalbero destro
        ...
        protected RecBST search(E label) {
            ...
            // Controlla se l'etichetta è nel sottoalbero sinistro o destro
            if (label.compareTo(this.label) < 0 && this.left != null) {
                // Se l'etichetta è minore, cerca nel sottoalbero sinistro
                return this.left.search(label);
            } else if (label.compareTo(this.label) > 0 && this.right != null) {
                // Se l'etichetta è maggiore, cerca nel sottoalbero destro
        ...
        protected void addLabelsInOrder(List<E> l) {
            // TODO implementare ricorsivamente
            // Visita in-order: sinistra - radice - destra
            if (this.left != null) {
                this.left.addLabelsInOrder(l); // Visita il sottoalbero sinistro
            }
            l.add(this.label);
            if (this.right != null) {
                this.right.addLabelsInOrder(l); // Visita il sottoalbero destro
            }
        }
        ...
        private void inOrderTraversal(RecBST node, List<E> result) {
            if (node != null) {
        ...
        protected RecBST getSuccessorNode() {
            // TODO implementare
            if (this.right != null) {
                // Se c'è un sottoalbero destro, il successore è il nodo più a sinistra nel sottoalbero destro
                return this.right.getMinNode();
            } else {
                // Il successore è il primo nodo a cui si risale dall'alto che è il figlio sinistro di un genitore
                RecBST current = this;
        ...
        protected RecBST getPredecessorNode() {
            // TODO implementare
            if (this.left != null) {
                // Se c'è un sottoalbero sinistro, il predecessore è il nodo più a destra nel sottoalbero sinistro
                return this.left.getMaxNode();
            } else {
                // Il predecessore è il primo nodo a cui si risale dall'alto che è il figlio destro di un genitore
                RecBST current = this;
                RecBST parent = this.parent;
        ...
        protected void deleteSelfLabel() {
            ...
            if (this.left == null && this.right == null) {
                ...
                if (this.parent != null) {
                    // Rimuovi il riferimento al nodo da suo genitore
                ...
                // Trova il successore o il predecessore, sostituisci l'etichetta di questo nodo con quella del successore/predecessore e cancella il nodo successore/predecessore
                RecBST successor = this.right.getMinNode(); // Puoi anche utilizzare il predecessore
                this.label = successor.label;
                successor.deleteSelfLabel(); // Cancella il successore/predecessore
            } else {
                // Caso 3: il nodo ha un solo figlio
                RecBST child = (this.left != null) ? this.left : this.right;
                if (this.parent != null) {
                    // Collega il figlio al genitore del nodo corrente
                    ...
                    // Se il nodo è la radice, il figlio diventa la nuova radice
